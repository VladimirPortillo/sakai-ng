...
export class AgregarRestauranteComponent {
  ...
  agregarRestaurante() {
    ...
    const restaurante: Restaurantes = {
      ...
      delivery: this.miFormulario.value.delivery,
      ...
      console.log('agregar_restaurante:',restaurante);

      this.restaurantesService.agregarRestauranteCompleto(restaurante,this.filesMulti).subscribe((resp) => {
        if(resp.ok) {
